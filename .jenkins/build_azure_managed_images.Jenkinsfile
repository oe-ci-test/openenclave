    ...
    node("nonSGX") {
        stage("${os_type}-${version}") {
            timeout(GLOBAL_TIMEOUT_MINUTES) {
                cleanWs()
                checkout scm
                def managed_image_name_id = sh(script: "git rev-parse --short HEAD", returnStdout: true).tokenize().last()
...
parallel "Build Ubuntu 16.04"              : { buildManagedImage("ubuntu", "16.04") },
         ...
         "Build Windows 2019 SGX1"         : { buildManagedImage("win", "2019-SGX") },
         "Build Windows 2019 SGX1FLC DCAP" : { buildManagedImage("win", "2019-SGX-DCAP") }
